...
def decode_texture_raw(raw_data, width, height, fmt):
    ...
    if fmt == 'BC3':
        ...
        for by in range(0, height, 4):
            for bx in range(0, width, 4):
                ...
                reds = [r0,r1,(6*r0+1*r1)//7,(5*r0+2*r1)//7,(4*r0+3*r1)//7,(3*r0+4*r1)//7,(2*r0+5*r1)//7,(1*r0+6*r1)//7] if r0>r1 else [r0,r1,(4*r0+1*r1)//5,(3*r0+2*r1)//5,(2*r0+3*r1)//5,(1*r0+4*r1)//5,0,255]
                for py in range(4):
                    for px in range(4):
                        x,y = bx+px, by+py
                        if x>=width or y>=height: continue
        ...
        for by in range(0, height, 4):
            for bx in range(0, width, 4):
                ...
                for ch, ch_off in enumerate([0, 8]):
                    ...
                    vals = [r0,r1,(6*r0+1*r1)//7,(5*r0+2*r1)//7,(4*r0+3*r1)//7,(3*r0+4*r1)//7,(2*r0+5*r1)//7,(1*r0+6*r1)//7] if r0>r1 else [r0,r1,(4*r0+1*r1)//5,(3*r0+2*r1)//5,(2*r0+3*r1)//5,(1*r0+4*r1)//5,0,255]
                    for py in range(4):
                        for px in range(4):
                            x,y = bx+px, by+py
                            if x>=width or y>=height: continue
...
def extract_groups_from_zip(zip_data, break_after_first=False):
    ...
    for t in tex_list:
        if 'png_data' not in t and t.get('format') != 'PNG' and t.get('width', 0) > 0:
            raw = base64.b64decode(t['data'])
            png_b64 = decode_texture_to_png_b64(raw, t['width'], t['height'], t['format'])
            if png_b64:
                t['png_data'] = png_b64
